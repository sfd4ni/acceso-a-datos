...
import es.iespuertodelacruz.daniel.matriculasrest.dto.UsuarioconrolDTO;
...
@Api(value= "V2REST", description = "REST APIs relacionadas con las entidades Alumno y Matricula")
@RestController
@RequestMapping("/api/v3/alumno")
public class V3REST {
	@Autowired
	...
	public ResponseEntity<?> getAllUsuarios(){
		List l = new ArrayList<UsuarioconrolDTO>();
		for (Usuarioconrol usuario : usuarioService.findAll()) {
			UsuarioconrolDTO usuarioDTO = new UsuarioconrolDTO(usuario);
			l.add(usuarioDTO);
		}
	...
	public ResponseEntity<?> getUsuarioByNombre(
			@PathVariable("nombre") String nombre) {
		
		Optional<Usuarioconrol> optUsuario = Optional.of(usuarioService.findByNombre(nombre));
	...
	public ResponseEntity<?> saveUsuario(
		...
		Usuarioconrol nuevoUsuario = null;
		try {
		...
		if (nuevoUsuario != null) {
			...
			return new ResponseEntity<>("Ya existe ese nombre", HttpStatus.CONFLICT);
		}
	}
	
	@DeleteMapping("/usuario/{nombre}")
	public ResponseEntity<?> deleteUsuario(@PathVariable String nombre){
		Optional<Usuarioconrol> optM = Optional.of(usuarioService.findByNombre(nombre));
		if(optM.isPresent()) {
			try {
				usuarioService.deleteById(optM.get().getIdusuario());
			} catch (Exception e) {
				e.printStackTrace();
				return new ResponseEntity<>("Error al borrar.", HttpStatus.CONFLICT);
			}
